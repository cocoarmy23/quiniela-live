...
def get_espn_scores(slug: str) -> dict:
    hoy    = datetime.now()
    fechas = [(hoy + timedelta(days=d)).strftime("%Y%m%d") for d in range(-1, 8)]
    ...
    for event in todos:
        ...
        result[eid] = {
            ...
            "detail":    detail,
            "completed": completed,
            "logo_home": (home.get("team", {}).get("logo") or ""),
            "logo_away": (away.get("team", {}).get("logo") or ""),
        }
    return result
...
def construir_bloque_filas(lista_partidos):
    ...
    for p in lista_partidos:
        ...
        loc_nombre = (p["local_nombre"] or "").upper()
        vis_nombre = (p["visita_nombre"] or "").upper()

        logo_h_html = f'<img src="{logo_home}" class="team-logo" onerror="this.style.display=\'none\'">' if logo_home else '<span class="logo-placeholder"></span>'
        logo_a_html = f'<img src="{logo_away}" class="team-logo" onerror="this.style.display=\'none\'">' if logo_away else '<span class="logo-placeholder"></span>'
        ...
        html_bloque += f'<div class="eq-local">{logo_h_html}<span class="eq-nombre">{loc_nombre}</span></div>'
        html_bloque += f'<div class="eq-visita">{logo_a_html}<span class="eq-nombre">{vis_nombre}</span></div>'
        html_bloque += '</td>'
...
.eq-local    { color:#e2e8f0; font-weight:600; font-size:12px; display:flex; align-items:center; gap:6px; white-space:nowrap; }
.eq-visita   { color:#94a3b8; font-size:11px; margin-top:4px; display:flex; align-items:center; gap:6px; white-space:nowrap; }
.eq-nombre   { overflow:hidden; text-overflow:ellipsis; }
